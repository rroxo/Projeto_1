...
####### explicação dos argumentos da funcao final_model##############

#modelos continuos (com várias particoes) vs. modelos binários 

#which_models = escolher qual modelo 
#raw_mean = calcula média de todos os modelos continuos
#raw_mean_th = gera modelo continuo final e a partir da media geral do tss vai gerar um modelo binário (1 e 0 a partir de um numero de corte)
#raw_mean
#bin_mean = media dos modelos binarios criado a partir do corte no modelos raw mean

#uncertainly = incertezas = precisão = quantidade de variação entre as partições geradas
#uncertainly = true: calcula a incerteza, gera um novo output, varia de 0 a 1.
#quanto maior a incerteza, maior a variação entre as particoes

#overwrite = substituir os arquivos, caso a funcao tenha sido rodada anteriormente